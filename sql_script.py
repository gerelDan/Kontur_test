...
big_query = 'select * ' \
            'from Bills LEFT JOIN (select * from Bill_content LEFT JOIN retail_packs ON ' \
            '(retail_packs.bcID = Bill_content.bcID)) AS query_1 ON (query_1.bID = Bills.id)'
#определяем время сейчас
now = datetime.now()
#Отправляем запрос
sql_3 = cursor.execute(big_query)
...
#Уберем пропуски
df = df.dropna()
#Оставим только продукт Контур-Экстерн
df = df[df['product'] == 'Контур-Экстерн']
...
#Еще раз оставим только такие счета где дата оплаты максимальная
